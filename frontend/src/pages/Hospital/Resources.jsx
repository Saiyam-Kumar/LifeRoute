...
export default function Resources() {
  ...
  const fetchHospital = async (showRefresh = false) => {
    try {
      ...
      setBeds(data.available_beds ?? 0);
      setIcu(data.available_icu ?? 0);
      setVentilators(data.ventilators ?? 0);
      ...
      setError(
        err.message || "Unable to load hospital resources right now."
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const toggleSpecialist = (specialist) => {
    setSpecialists((current) =>
      current.includes(specialist)
        ? current.filter((item) => item !== specialist)
        : [...current, specialist]
    );
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setError(
        err.message || "Unable to update hospital resources."
      );
    } finally {
      setSaving(false);
    }
        ...
        <header className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/40">
              Keep your hospital's capacity and emergency resources up to
              date. These values help LifeRoute make better routing decisions.
            </p>
          </div>
        ...
        <section className="mt-8 rounded-2xl border border-white/[0.08] bg-[#11151D] p-6">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            ...
            >
              ...
              >
                {isOpen ? "Currently Operational" : "Currently Closed"}
              </span>
          ...
          <div className="mt-4 rounded-2xl border border-white/[0.08] bg-[#11151D] p-6">
            <div className="flex flex-wrap gap-2.5">
              {SPECIALIST_OPTIONS.map((specialist) => {
                const selected = specialists.includes(specialist);
            ...
            <div className="mt-5 flex items-center gap-2 border-t border-white/[0.06] pt-4">
              <Stethoscope size={14} className="text-white/25" />

              <span className="text-xs text-white/30">
                {specialists.length} specialist
                {specialists.length === 1 ? "" : "s"} currently available
              </span>
            </div>
          </div>
        </section>
          ...
          <div className="mt-4 rounded-2xl border border-white/[0.08] bg-[#11151D] p-6">
            <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
              {RESOURCE_OPTIONS.map((resource) => {
                const selected = resources.includes(resource);
                ...
                return (
                  ...
                  >
                    ...
                    {selected ? (
                      ...
                      <X size={14} className="shrink-0 text-white/15" />
                    )}
                  </button>
                );
              })}
            ...
            <div className="mt-5 flex items-center gap-2 border-t border-white/[0.06] pt-4">
              ...
              <span className="text-xs text-white/30">
                ...
                {resources.length === 1 ? "" : "s"} currently available
              </span>
            </div>
          </div>
        </section>
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-white/[0.09] bg-[#0F131A]/95 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-white/30">
                Updated resources can affect LifeRoute's hospital
                recommendations.
              </p>
            </div>
...
function NumberResourceCard({
  ...
  const accents = {
    orange: {
      icon: "bg-[#FF5A36]/[0.08] text-[#FF5A36]",
      focus: "focus:border-[#FF5A36]/30 focus:ring-[#FF5A36]/10",
    },
    blue: {
      icon: "bg-[#7C8CF5]/[0.08] text-[#7C8CF5]",
      focus: "focus:border-[#7C8CF5]/30 focus:ring-[#7C8CF5]/10",
    },
    green: {
      icon: "bg-emerald-400/[0.08] text-emerald-400",
      focus: "focus:border-emerald-400/30 focus:ring-emerald-400/10",
    },
  };
